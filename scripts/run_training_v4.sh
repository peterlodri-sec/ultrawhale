...
HF_TOKEN=${HF_TOKEN:-}
HF_REPO=${HF_REPO:-"PeetPedro/kompress-v4"}
...
for i, r in enumerate(records):
    ...
    if ratio >= 1.2 and len(new_ref) >= 30:
        out.append({
            ...
            "source": "self_labeled_v3_override",
            "topic": r.get("topic",""),
        })
    else:
        skipped += 1
...
load_v2_weights(model, "kompress-v4-finetuned")
model.eval()
...
class W(torch.nn.Module):
    ...
    def forward(self,i,a):
        ...
        return p*(0.5+0.5*s)
os.makedirs("kompress-v4-finetuned/onnx",exist_ok=True)
torch.onnx.export(W(model),(dummy["input_ids"],dummy["attention_mask"]),
    "kompress-v4-finetuned/onnx/kompress-fp32.onnx",
...
api.upload_folder(folder_path="kompress-v4-finetuned",repo_id="${HF_REPO}",
    commit_message="kompress-v4: self-labeled references via v3+override")

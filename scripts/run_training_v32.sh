...
#   - Merges kompress_train_split + kompress_multi_train + domain_train into v32_train
#   - Uses train_kompress_v32.py (must_keep_weight=6.0, hard label override)
#   - Fine-tunes from PeetPedro/kompress-v3 (not v2-base)
#   - Uploads to PeetPedro/kompress-v32
set -euo pipefail

cd /workspace/ultrawhale
HF_TOKEN=${HF_TOKEN:-}
HF_REPO=${HF_REPO:-"PeetPedro/kompress-v32"}
...
python3 scripts/train_kompress_v32.py \
    --data data/v32_train.jsonl \
    --base-model PeetPedro/kompress-v31 \
    --output kompress-v32-finetuned \
    --epochs 3 \
    --batch-size 16

echo "=== 4/5 Eval ==="
python3 scripts/eval_kompress.py \
    --model kompress-v32-finetuned \
...
import torch
from scripts.train_kompress_v32 import HeadroomCompressorModel, load_v2_weights
from transformers import AutoTokenizer

model = HeadroomCompressorModel("answerdotai/ModernBERT-base")
load_v2_weights(model, "kompress-v32-finetuned")
model.eval()
...
os.makedirs("kompress-v32-finetuned/onnx", exist_ok=True)
torch.onnx.export(
    wrapped, (ids, mask),
    "kompress-v32-finetuned/onnx/kompress-fp32.onnx",
...
print("ONNX exported to kompress-v32-finetuned/onnx/kompress-fp32.onnx")
PY
...
api.upload_folder(
    folder_path="kompress-v32-finetuned",
    repo_id="${HF_REPO}",
    commit_message="kompress-v32: must_keep_weight=6.0, hard label override, domain data",
)
print("Uploaded to huggingface.co/${HF_REPO}")
PY
else
    echo "HF_TOKEN not set, skipping upload. Model in: kompress-v32-finetuned/"
fi

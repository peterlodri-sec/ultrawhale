...
#   - Uses train_kompress_v33.py (must_keep_weight=6.0, hard label override)
...
HF_TOKEN=${HF_TOKEN:-}
HF_REPO=${HF_REPO:-"PeetPedro/kompress-v33"}
...
files = [
    "data/domain_train_2k.jsonl",
    "data/domain_train.jsonl",
]

pathlib.Path("data").mkdir(exist_ok=True)
out_path = "data/v33_train.jsonl"
total = 0
...
python3 scripts/train_kompress_v33.py \
    --data data/v33_train.jsonl \
    --base-model PeetPedro/kompress-v32 \
    --output kompress-v33-finetuned \
    --epochs 3 \
    --batch-size 16

echo "=== 4/5 Eval ==="
python3 scripts/eval_kompress.py \
    --model kompress-v33-finetuned \
...
load_v2_weights(model, "kompress-v33-finetuned")
model.eval()
...
os.makedirs("kompress-v33-finetuned/onnx", exist_ok=True)
torch.onnx.export(
    wrapped, (ids, mask),
    "kompress-v33-finetuned/onnx/kompress-fp32.onnx",
...
print("ONNX exported to kompress-v33-finetuned/onnx/kompress-fp32.onnx")
PY
...
api.upload_folder(
    folder_path="kompress-v33-finetuned",
    repo_id="${HF_REPO}",
...
PY
    ...
    echo "HF_TOKEN not set, skipping upload. Model in: kompress-v33-finetuned/"
fi

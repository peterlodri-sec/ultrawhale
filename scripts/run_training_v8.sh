#!/usr/bin/env bash
# Kompress v8: C3 self-distillation — train on Qwen2.5-7B labeled real tool outputs
#
...
HF_TOKEN=${HF_TOKEN:-}
HF_REPO=${HF_REPO:-"PeetPedro/kompress-v8"}

echo "=== 1/5 Convert Qwen labels to token keep/drop ==="
...
python3 scripts/train_kompress.py \
    --data data/kompress_v8_train.jsonl \
...
python3 scripts/eval_heretic.py \
    --model kompress-v8-finetuned \
    --prompts-file data/heretic_expanded.jsonl || echo "WARN: eval non-fatal"

echo "=== 5/5 ONNX export + HuggingFace upload ==="
...
load_v2_weights(model, "kompress-v8-finetuned")
model.eval()
...
os.makedirs("kompress-v8-finetuned/onnx", exist_ok=True)
torch.onnx.export(
    Wrapper(model),
    (dummy["input_ids"], dummy["attention_mask"]),
    "kompress-v8-finetuned/onnx/kompress-fp32.onnx",
...
api.upload_folder(
    folder_path="kompress-v8-finetuned",
    repo_id=repo,
    commit_message="kompress-v8: C3 self-distillation — Qwen2.5-7B teacher labels on real tool outputs"
)
